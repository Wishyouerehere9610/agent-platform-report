    ...
    "controlProfiles": [
      {
        ...
        "browser": {
          "summary": "浏览器能力嵌在飞书和豆包工作里，适合在飞书文档、多维表格和 Web 视图之间抓取、填表并回写结果。",
          "evidence": [
            "OFF-DB-005",
            "RES-CONTROL-001"
          ]
        ...
        "computer": {
          "summary": "电脑操控走虚拟桌面和云电脑双模式，适合后台长任务、批量任务和跨设备接力。",
          "evidence": [
            "OFF-DB-005",
            "SOC-DB-001",
            "RES-CONTROL-001"
        ...
        "implementation": {
          "browser": "Agent 在飞书客户端的侧边栏运行，能打开飞书 Web 视图、读取飞书文档和多维表格，也能拉起虚拟桌面里的浏览器做搜索、比价、填表和结果回写；不需要额外安装浏览器扩展。",
          "computer": "豆包工作在 2026 年 8 月连续补齐电脑操控：8 月 17 日上线手机远程控制电脑，8 月 18 日推出 Windows 虚拟桌面，8 月 19 日新增本地和云电脑双模式。云电脑可以在用户关机后继续跑定时、长周期和批量任务。",
          "environment": "执行环境在本地电脑、虚拟桌面和云电脑之间切换；飞书入口负责组织上下文和结果写回，云电脑负责不抢屏的长任务。"
        },
        "strengths": [
          "虚拟桌面加云电脑双模式是三家里最完整的产品形态，既能处理本地任务，也能把长任务交给云端继续跑。",
          "任务跑在虚拟桌面时不会抢用户的鼠标键盘，后台体验比直接接管前台软件更自然。",
          "飞书身份、组织上下文、文档和多维表格连接最顺，抓取后的结果能直接进入协作空间。",
          "Seedream 和 Seedance 能补上图片、视频素材生产，适合把信息采集延伸到营销物料。"
        ],
        "limitations": [
          "复杂 GUI、多表关联公式和模板填表仍需要人工复核，本次交付也出现过 PPT 与 XLSX 分数不一致。",
          "额度按滚动时间窗和周额度计算，用户很难在任务前准确估算一次长任务会消耗多少资源。",
          "飞书之外的组织上下文优势会明显减弱，跨到其他办公生态时需要额外连接器和权限配置。"
        ],
        "timeline": [
          {
            "date": "2026-08-17",
      ...
      },
      {
        ...
        "browser": {
          "summary": "浏览器能力分本地和云端两条路径，本地复用登录态处理内网和 SaaS 后台，云端浏览器池承担批量抓取。",
          "evidence": [
            "RES-CONTROL-001",
            "OBS-WB-001"
          ]
        ...
        "computer": {
          "summary": "电脑操控是 OS 级视觉加键鼠事件，适合接管没有 API 的桌面软件、ERP 和内网系统。",
          "evidence": [
            "RES-CONTROL-001"
          ]
        },
        "implementation": {
          "browser": "本地模式通过 BrowserSkill 或 camofox-browser 接管用户已经登录的 Chromium 会话，保留 Cookie，减少反爬和重复登录问题；云端模式使用腾讯云端浏览器池，适合批量网页采集和长跑任务。",
          "computer": "WorkBuddy 不走应用 API，而是截图、识别屏幕并模拟鼠标键盘。这个路径可以绕开企业微信、金蝶、内网 ERP 等老系统的接口限制，用户也能在微信或企业微信里发一句话触发电脑端执行。",
          "environment": "本地沙箱负责桌面和文件任务，云端 Managed Agents 负责托管、多 Agent 并行和长任务续跑；同一个任务里可以混用本地浏览器、云端浏览器和桌面操控。"
        },
        "strengths": [
          "Computer Use 工程最扎实，适合接管没有 API 的老软件和内网系统。",
          "本地浏览器可以复用 Cookie 和登录态，处理已登录 SaaS 后台时少走一次授权。",
          "13 个模型可以切换，混元、DeepSeek、Claude、Kimi 和通义等模型能按任务选择。",
          "微信一句话触发降低了使用门槛，非技术员工更容易把它当成日常执行入口。"
        ],
        "limitations": [
          "连接器登录态、文件路径和版本更新会影响稳定性，出问题时需要先检查连接器是否断开。",
          "联网搜索时效性不如专门的信息检索产品，适合执行任务，不适合当最终信息源。",
          "积分消耗不够透明，一次深度任务可能消耗数百积分，任务前不容易估算成本。",
          "大版本更新后曾出现工作空间文件夹消失和开关重置，企业使用需要配置变更检查。"
        ],
        "timeline": [
          {
            "date": "2026-06-05",
      ...
      },
      {
        ...
        "browser": {
          "summary": "浏览器能力嵌在云端 Agent 里，优势在阿里电商数据和钉钉工作流，不是独立插件路线。",
          "evidence": [
            "OFF-QW-007",
            "OFF-QW-009",
            "RES-CONTROL-001"
        ...
        "computer": {
          "summary": "桌面操控覆盖 Mac、Windows 和鸿蒙客户端，也支持本地安全工作环境里的后台执行。",
          "evidence": [
            "OFF-QW-004",
            "RES-CONTROL-001"
          ]
        ...
        "implementation": {
          "browser": "Browser Use 内置在云端 Agent 中，Night Plan 可以在晚上 10 点到早上 8 点错峰跑批处理。淘宝、1688、闲鱼和小红书等数据源让电商团队能把选品、比价、复盘放在一条链路里跑。",
          "computer": "千问办公有 Mac、Windows 和鸿蒙桌面客户端，鸿蒙版补齐了国产系统入口。官方 Computer Use 文档记录了屏幕感知、完整键鼠操作和后台执行能力，但当前未见独立云电脑形态。",
          "environment": "网页端、桌面客户端和钉钉入口并行；云端 Agent 适合跑网页和电商数据任务，桌面端在本地安全工作环境里访问文件和应用。"
        },
        "strengths": [
          "淘系电商数据源是最清楚的差异点，淘宝、1688、闲鱼和小红书数据更适合电商选品和复盘。",
          "桌面客户端覆盖 Mac、Windows 和鸿蒙，国产终端覆盖比另外两家更完整。",
          "钉钉 25 项原生能力打通，适合已经把审批、消息和文档放在钉钉里的团队。",
          "金融、法务、电商和咨询专家套件较完整，业务同学可以直接从行业模板开始。"
        ],
        "limitations": [
          "当前未见独立云电脑或远程虚拟桌面产品形态，后台长任务边界不如豆包云电脑清楚。",
          "重执行任务需要浏览器能力和桌面客户端组合兜底，复杂 GUI、验证码和保存流程仍要人工校验。",
          "当前公开资料缺少同等级的具名外部客户，行业覆盖更多来自官方场景和能力推断。"
        ],
        "timeline": [
          {
            "date": "2026-08-03",
        ...
        "bestFor": "已使用钉钉，且业务重点在电商研究、淘系数据、金融法务专家套件或轻量办公交付的团队。",
        "limit": "当前未见独立云电脑形态，重桌面执行和长任务后台能力需要先做小样验证。",
