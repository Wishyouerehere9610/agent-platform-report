# WorkBuddy 单次实测 Prompt

你正在执行一次办公 Agent 单次运行测评。请在 **Auto 模式** 下独立完成以下任务，最长执行 45 分钟；全程不要切换模式，也不要向任何同事、群聊、客户或外部联系人发送消息。
...
`%USERPROFILE%\Desktop\行业调研\outputs\workbuddy`

不得写入千问办公的输出目录，不得覆盖输入文件。
...
仅在企业微信或腾讯办公生态的**本人可见范围**完成，禁止联系任何真实同事或群：

1. 向自己的个人消息入口或文件传输助手发送不超过 200 字的管理层摘要，包含两个优先行业和本地输出目录。
2. 创建个人待办：`审阅 FDE 商业化方案`。
3. 创建次日 30 分钟个人日历提醒：`审阅 FDE 商业化方案`，不邀请任何人。
4. 如个人文件空间可用，创建 `FDE评测-WorkBuddy` 文件夹并保存或链接交付物；若不可用，记录失败原因后继续。
...
- 产品版本、模式、开始时间、首个可见成果时间、核心任务完成时间、通信测试完成时间、结束时间。
...
- 最终回复只汇总产物路径、完成状态、耗时、Credits、通信连通结果和主要失败点。
