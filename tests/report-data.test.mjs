...
test("Doubao deep dive keeps product history, integration and commercial judgment evidence linked", () => {
  ...
  assert.equal(deepDive.commercialOpportunities.length, 4);
  ...
  assert.ok(deepDive.commercialOpportunities.some((item) => item.title === "企业 Agent 交付"));
  assert.ok(!deepDive.commercialOpportunities.some((item) => ["飞书 AI 加购", "生产力全栈", "跨生态与生产交付"].includes(item.title)));
