# 05-communication-check.md — 通信连通性测试记录

> 数据校正（2026-08-26）：飞书属于字节跳动，不属于腾讯办公生态。飞书 4/4 是替代通道结果，不计入企业微信目标成绩。

## 测试环境

- **替代通道**: 飞书（Feishu / Lark），4/4 动作完成
- **企业微信（WeCom）状态**: 未连接（disconnected），4 项目标动作未完成
...
| 00:07 | 用户决定停止钉钉授权 → 停 dws 进程（task IlQrzS killed，端口 49885 关闭），钉钉未实际授权；飞书 4/4 保留为替代通道结果 |
| 00:08 | 补全两份日志的全流程记录 |
...
| 测试对象 | 结果 | 说明 |
|---|---|---|
| **飞书 Lark（替代通道）** | ✅ **4/4 成功，不计入企业微信成绩** | 待办、日历、自我消息、云盘文件夹+9文件上传 |
| 企业微信 WeCom | ❌ 当前工具路径不适用 | wecom-cli 走 Bot 创建链路（扫码落"建立企业智能机器人"页报"参数不合法"），没有个人待办/日历/云盘权限 |
| 钉钉 DingTalk | ⚪ 能力就绪但未采用 | dws v1.0.59 含 calendar/chat/drive，00:05 已发起授权并生成二维码，但用户最终选飞书；且严格不属"腾讯办公生态" |

**企业微信目标结果 = 0/4；飞书替代通道 = 4/4：**
...
### 4 个企业微信动作（当前工具路径不适用，未执行）

> 原拟用 wecomcli-message / -todo / -calendar / -disk 执行 4 个动作，但 wecom-cli 是 Bot 创建工具、无用户级资源权限，且扫码落"建立企业智能机器人"页报"参数不合法"。该路径未完成，飞书 4/4 只作为替代通道记录。
